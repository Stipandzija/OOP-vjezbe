...
int main() {
	...
	vector<string> R_shema = { "ABCDEFGHIJ","ABCDEFGHIJK","ABCDEFGHIJKLMNO","ABCDEFGHIJKLMNOP","RaArKuMjOa" };
	vector<vector<string>> F_min = { {"DI->B" , "AJ->F" , "GB->FJE" , "AJ->HD" , "I->CG"}, {"A->D","BD->E","C->B", "D->GEHK", "DI->ABEC", "BHK->JA", "AI->K"}
	,{"AB->EF","CD->FGEH","EO->AHIJK", "L->MN", "N->ABCDEF", "KM->GHIAC", "F->MNO"},{"A->A","A->B","B->A","B->B", "CD->AKL", "MN->NOP", "D->M"},{"RaAr->Ku" , "Ar->Oa" , "Ku->Mj"} };
	...
	while (izvrsavanje == true) //program se izvrsava dok je true
	{
		...
		if (odabir > (R_shema.size() + 3)) {
			...
			vector<string> sss;
			vector<string> v = kanonski(F_min, odabir, svi_lijevi, svi_desni, sss);
			string v2 = vracanje(v, R_shema[odabir - 1]);
			vector<string> v3 = kljucevi_valjda(v); // svi ljevi
			...
			treca_norm(rjesenja(v4, svi_lijevi, svi_desni, sss, R_shema[odabir - 1]),svi_lijevi,svi_desni,R_shema[odabir-1]);
